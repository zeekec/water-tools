...
get_flow_data <- function(siteNo, startDate = "1895-02-01", endDate = Sys.Date()) {
    # statistics of interest - this is mean daily discharge
    pCode <- "00060"
    statCd <- "00003"

    # retrieve data
    gauge <- as_tibble(gauge <- readNWISdv(
        siteNo, pCode, startDate, endDate
    ))
...
# guage_function("08330000", "1895-01-01")

guage_function <- function(siteNo,
                           startDate = "1895-02-01", endDate = Sys.Date(),
                           plotYear = format(Sys.Date(), "%Y")) {
    previousYear <- as.character(as.integer(plotYear) - 1)
    ...
    subtitle <- paste(
        "Daily flows\nUSGS gauge ",
        gauge_meta$site_no,
        "\nData series start date: ", startDate
    )

    this_year <- filter(gauge_daily, year == plotYear)
    last_year <- filter(gauge_daily, year == previousYear)
    ...
    p <- ggplot(gauge_summary, aes(yday)) +
        ...
        geom_path(data = this_year, aes(x = yday, y = flow + 1, color = plotYear)) +
        geom_path(data = last_year, aes(x = yday, y = flow + 1, color = previousYear)) +
        ...
        labs(
            ...
            x = "Day of the Year",
            y = expression(paste("Flow (feet"^3, " / second)")),
            subtitle = subtitle
        ) +
